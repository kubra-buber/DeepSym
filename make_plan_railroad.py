...
def transition_safe(state: State, action) -> List[Tuple[State, float]]:
    ...
    Railroad's get_usable_actions can be permissive for some Python-side
    operators/negated fluents. transition() is the final authority. If it says
    a precondition is not satisfied, this action is simply not usable in this
    state, so the expected-reachability search should skip it rather than crash.
    """
    try:
        outcomes = transition(state, action)
    except RuntimeError as exc:
        if "Precondition not satisfied" in str(exc):
            return []
        raise
