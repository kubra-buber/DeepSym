...
def load_model(run_dir: Path, checkpoint_ext: str):
    ...
    module = importlib.import_module("models_vq_dynamic")
...
def main() -> None:
    ...
    if args.experiments:
        ...
        experiment_dirs = sorted(
            ...
            if path.is_dir() and path.name.startswith("l1_")
        )
    ...
    for experiment_dir in experiment_dirs:
        ...
        for run_dir in sorted(experiment_dir.glob("seed_*")):
            try:
                ...
                continue
            if not (run_dir / "encoder1_best.ckpt").exists():
                continue
        ...
        plot_experiment_curves(experiment_dir, seed_dirs, args.dpi)
