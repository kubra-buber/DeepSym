...
// GroundedEffect
  ...
  py::enum_<GoalType>(m, "GoalType")
      ...
      .value("NUMERIC", GoalType::NUMERIC);
  ...
  py::class_<GoalBase, GoalPtr>(m, "Goal")
      ...
      .def(
          ...
          py::arg("state"),
          "Check if goal is satisfied by a full State")
      ...
      .def(
          ...
          [](const GoalBase &goal,
             const std::unordered_set<Fluent> &fluents) {
            return goal.goal_count(fluents);
          },
          py::arg("fluents"),
          "Count satisfied goal leaves using fluent state")
      .def(
          "goal_count",
          [](const GoalBase &goal,
             const State &state) {
            return goal.goal_count(
                state.fluents(),
                state.numeric_values());
          },
          py::arg("state"),
          "Count satisfied goal leaves using a full State")
  ...
  py::class_<
      NumericGoal,
      GoalBase,
      std::shared_ptr<NumericGoal>>(
          m,
          "NumericGoal")
      .def(
          py::init<NumericCondition>(),
          py::arg("condition"))
      .def_property_readonly(
          "condition",
          &NumericGoal::condition)
      .def(
          "__repr__",
          [](const NumericGoal &g) {
            return "NumericGoal(" +
      ...
      .def(py::pickle(
          ...
          [](py::tuple t) {
            if (t.size() != 1) {
              throw std::runtime_error(
                  "Invalid state for NumericGoal!");
            }

            return std::make_shared<NumericGoal>(
                t[0].cast<NumericCondition>());
          }));
