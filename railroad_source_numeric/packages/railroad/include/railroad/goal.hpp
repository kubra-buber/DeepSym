...
enum class GoalType { LITERAL, AND, OR, TRUE_GOAL, FALSE_GOAL, NUMERIC };
...
public:
  ...
  // This preserves the original fluent-only Goal API.
  virtual bool evaluate(const FluentSet& fluents) const = 0;

  // State-aware evaluation. Existing fluent-only goals inherit the
  // default behavior. NumericGoal and composite goals containing
  // NumericGoal override this version.
  virtual bool evaluate(const FluentSet& fluents,
                        const NumericState& numeric_values) const {
    (void)numeric_values;
    return evaluate(fluents);
  }
  ...
  // - TRUE/FALSE: 0
  int goal_count(const FluentSet& fluents) const {
    return satisfied_leaf_count(fluents);
  }

  int goal_count(const FluentSet& fluents,
                 const NumericState& numeric_values) const {
    return satisfied_leaf_count(fluents, numeric_values);
  }
...
protected:
  ...
  virtual int satisfied_leaf_count(
...
// NumericGoal: leaf numeric condition evaluated against State::numeric_values.
class NumericGoal final : public GoalBase {
public:
  explicit NumericGoal(NumericCondition condition)
      : condition_(std::move(condition)) {}

  // A numeric goal cannot be evaluated correctly without numeric state.
  bool evaluate(const FluentSet&) const override {
    throw std::runtime_error(
        "NumericGoal requires numeric state values; "
        "use state-aware evaluation");
  }

  bool evaluate(
      const FluentSet&,
      const NumericState& numeric_values) const override {
    return condition_.evaluate(numeric_values);
  }

  GoalType get_type() const override {
    return GoalType::NUMERIC;
  }

  GoalPtr normalize() const override {
    return std::make_shared<NumericGoal>(condition_);
  }

  // The current FF heuristic operates on Fluent literals only.
  FluentSet get_all_literals() const override {
    return {};
  }

  const NumericCondition& condition() const {
    return condition_;
  }

  bool equals(const GoalBase& other) const override {
    if (other.get_type() != GoalType::NUMERIC)
      return false;

    const auto& o =
        static_cast<const NumericGoal&>(other);

    return condition_ == o.condition_;
  }
...
protected:
  ...
  int satisfied_leaf_count(
      const FluentSet&) const override {
    throw std::runtime_error(
        "NumericGoal requires numeric state values; "
        "use state-aware goal_count");
  }
  ...
  std::vector<FluentSet>
  compute_dnf_branches() const override {
    // Numeric conditions are deliberately ignored by the
    // current fluent-only FF heuristic. An empty fluent
    // conjunction therefore gives zero heuristic cost.
    //
    // Exact goal evaluation still checks the numeric condition.
    return {{}};
  }

private:
  NumericCondition condition_;
};
